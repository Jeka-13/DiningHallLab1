...
namespace DinningHall.Models
{
    public class Table
    {
        ...
        public void GetClients()
        {
            Task.Run(() =>
            {
                var time = RandomGenerator.GenerateTime(Configurator.TABLE_WAIT_FOR_CLIENTS_MAX);
                Thread.Sleep(time);
        ...
        public Order GetOrder(Waiter waiter, List<Food> menu)
        {
            ...
            var time = RandomGenerator.GenerateTime(Configurator.TABLE_MAKING_ORDER_MAX, Configurator.TABLE_MAKING_ORDER_MIN);
            Thread.Sleep(time);
